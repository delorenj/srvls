...
- **Rule:** a fixed collection pool uses deterministic longest-processing-time
  ...
  5. in worker-ID order, spawn every still-before-deadline member, establish its
     dedicated process group, and drive the concurrent AD-25 Hello/Ready
     handshake; every member reaches authenticated-ready or one terminal AD-25
     failure before any batch request is dispatched;
  6. after those outcomes, when a still-live process-scope member is
     authenticated-ready and strictly before both deadlines, close the
     worker-spawn gate, then resolve every coordinator-owned AD-13
     UnrootableSpawnV1 from current or superseded generations. Each must have
     its exact owned child reaped and any known
     dedicated group proven empty before the process request. An earlier process
     worker failure uses its normal AD-25 report; if absence is still unproved
     at either deadline, the process member receives `worker-timeout`. Both
     paths perform no Host-read and reopen the gate. Only after that absence
     proof freeze every
     complete existing and batch spawned root whose group is not proven empty;
  7. dispatch WorkerRequestV1 to each authenticated-ready member that remains
     strictly before its scope and generation deadlines, in worker-ID order;
     a member whose deadline elapsed while the batch resolved instead receives
     the one AD-25 `worker-timeout` report without a request;
  ...
  9. when the half-open process cut closes, or the process member terminalizes
     before Host-read, reopen the gate and begin the next epoch with every
     then-free slot.

  The coordinator sends every Hello and observes every Ready or terminal
  outcome concurrently against the absolute member deadline; a silent child,
  exit 77, malformed or replayed Ready, failed peer proof, or setup failure
  cannot hold the batch past the latest member deadline or generation cutoff.
  AD-25 synthesizes one terminal report for each failed member; ready members
  still dispatch in slot order if time remains. A failed member with a complete
  ...
  worst-case simulation: every scope's complete setup-plus-work lane consumes
  its full configured deadline; successful setup consumes part of rather than
  adds to that deadline; the process scope closes worker spawn only for its
  remaining Host-read interval through its absolute deadline, with zero-cost
  successful setup as the admitted full-budget worst case; slots that
  finish under the barrier remain idle; and queued dispatch resumes in LPT
  order only when the barrier opens. Because report admission is half-open, it
...
- **Rule:** the frozen Python compatibility corpus and golden outputs cover
  ...
  Host reads; a zero-margin lane proves the mandatory one-nanosecond half-open
...
- **Rule:** `PromiseId`, `SnapshotId`, `PlanId`, `OperationId`, and lifecycle
  ...
  The coordinator has therefore resolved every batch Ready/failure outcome and
  every unrootable-child absence barrier, then snapshots every complete existing
  and batch spawned group not proven empty before releasing any request. The
  gate remains closed through the half-open process Host-read cut. A later
...
- **Rule:** the parent launches the exact current executable with sole raw argv
  ...
  Only after every batch member is Ready or terminal may the parent send
  WorkerRequestV1 to the still-before-deadline Ready members. Request ID plus
...
| UJ-5 | AD-5, AD-16, AD-18, AD-20–AD-21; exact duplicate evidence and retained timestamped resource history |
